        ...
        @foreach ($data_item->voteProfiles as $v)
            @foreach (json_decode($v->gallery) as $g)
                ...
                <td>@if($i === $g)
                    ...
                    <br>
                    {{$g}}
                    <button wire:click="kill({{ $i }})"
                        class="btn-danger text-white w-32 px-4 py-1 hover:bg-red-600 rounded border">Sure?</button>
                @else
                @php
                    $data = $v->g;
                @endphp
                    <button wire:click="confirmDelete({{ $v->gallery }})"
                        class="btn-secondary text-dark w-32 px-4 py-1 hover:bg-red-600 rounded border">Delete</button>
                @endif
            </td>
            @endforeach
        @endforeach
